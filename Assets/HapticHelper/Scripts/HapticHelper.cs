...
public class HapticHelper : MonoBehaviour {
   ...
   public void Update()
   {
       if (TestMode)
       {
            if (OVRInput.GetDown(ClipButton, myController))
           {
               if (myController == OVRInput.Controller.LTouch)
               {
                   PlayHapticAudioClip(true, ClipToTest, true);
               }
               else
               {
                    PlayHapticAudioClip(false, ClipToTest, true);
               }
           }
           if(continuous)
           {
               //Keep The Length low so as not to stress out the processor
               if (OVRInput.Get(ProceduralButton, myController))
               {
                   if (myController == OVRInput.Controller.LTouch)
                   {
                       if(useSin)
                       {
                           GenerateSinPulse(true, pIntensity, SinSpeed, pLength);
                       }
                       else
                       {
                           ProceduralTone(true, pIntensity, pLength);
                       }
           ...
           {
               if (OVRInput.GetDown(ProceduralButton, myController))
               {
                   if (myController == OVRInput.Controller.LTouch)
                   {
                       ProceduralTone(true, pIntensity, pLength);
